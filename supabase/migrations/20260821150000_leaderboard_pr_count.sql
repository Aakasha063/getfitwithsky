...
DROP FUNCTION IF EXISTS private.leaderboard(integer);

CREATE FUNCTION private.leaderboard(period_days integer DEFAULT 30)
RETURNS TABLE(user_id uuid, display_name text, avatar_url text, avatar_color text, sessions integer, sets_count integer, total_volume numeric, last_session date, active_weeks integer, pr_count integer)
LANGUAGE sql
...
CREATE FUNCTION public.get_leaderboard(period_days integer DEFAULT 30)
RETURNS TABLE(user_id uuid, display_name text, avatar_url text, avatar_color text, sessions integer, sets_count integer, total_volume numeric, last_session date, active_weeks integer, pr_count integer)
LANGUAGE sql
